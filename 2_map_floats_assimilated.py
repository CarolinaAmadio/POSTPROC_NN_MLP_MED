...
N3n     = dfqc.loc[( dfqc.Qc=='B' ) | (dfqc.Qc=='I')]
N3n_rec = dfqc.loc[( dfqc.Qc=='P' )]
LIST_COL        =  ['lon','lat','DATE','NAME']

CHLA.dropna(inplace=True)
O2o.dropna(inplace=True)
N3n.dropna(inplace=True)
N3n_rec.dropna(inplace=True)
...
# Oxy
lon,lat = np.array(O2o.O2o_LON), np.array(O2o.O2o_LAT)
lons, lats      = map( lon,lat)  # transform coordinates
scat = ax.scatter(lons, lats,          s=10, zorder=4, color='k', marker= ".", edgecolor='k', linewidth=0.9 , alpha=1)
scat = ax.scatter(lons[0], lats[0],    s=10, zorder=4, color='k', marker= ".", edgecolor='k', linewidth=0.9 , alpha=1, label='O2o')

ax.annotate(text = "Alb",xy  = (map(-4.8,  35.8) ), fontsize=16,  weight='bold') #,bbox={'facecolor': 'w', 'alpha': 0.5, 'pad': 10}  ))
ax.annotate(text = "Swm1",xy = (map(1.2,     37.45)   ), fontsize=16, weight='bold' )
...
ax.annotate(text = "Adr1",xy = (map(14.3,  43.5)   ), fontsize=16, weight='bold')
